...
- **Model conversion is user-managed.** The CoreML models (~6.5 GB total: UNet, VAE encoder/decoder, two text encoders) are not auto-downloaded. Users convert them via the recipe in the CoreML SDXL Phase 3 plan status (`~/.claude/plans/coreml-sdxl-phase3.md`): clone `apple/ml-stable-diffusion` (tag 1.1.1), apply the `coremltools` `_cast` patch, install torch 2.7 + coremltools 9, convert with `--custom-vae-version madebyollin/sdxl-vae-fp16-fix` (mandatory: the SDXL base VAE overflows in fp16 and darkens every decode by ~15/255 without it), and run `scripts/gen_coreml_sd_empty_prompt.py`. This is a dev-only step; the release binary ships with no models and has no download logic.
