...
- **Tiled img2img preserves up to 4K.** Tiles overlap and cosine-feather so seams are not visible; `--regen-no-tile` forces whole-image (fails above ~1024px on most GPUs). Knobs: `--regen-strength` (0.01 to 0.30, default 0.05), `--regen-steps` (default 20), `--regen-no-download`, `--regen-model-path` / `--regen-vae-path`.
