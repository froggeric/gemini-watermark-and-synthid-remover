...
- **The 48px still mask is confirmed optimal and unchanged**: independent measurements on real 48px-marked images (a black-background capture, a mid-background Gemini 3.8 image, and five textured 2400x1792 paintings) bracket its plateau within measurement noise; its own floor is 0.0008 (negligible). Known remaining limit, documented for future work: measured alpha decreases slightly with background level (0.305 on black vs 0.264 on a flat 230-gray band), consistent with the overlay color being slightly below pure white (fit: alpha ~0.295, overlay ~250/255, R2 0.42 — too weak to act on). A two-parameter (alpha, overlay-color) exact inversion would remove the residual +-1.5/255 background-dependent bias if ever confirmed with better data.
