...
- **Tradeoffs, stated plainly.** It is lossy (typical output 38 to 45 dB PSNR versus the input). It downloads ~6.5 GB (SDXL base checkpoint) + ~335 MB (fp16-fix VAE) on first use, SHA256-pinned to the HuggingFace LFS content oids and cached in `~/.cache/wmr/`; the model and VAE are NOT bundled. It leaves a detectable diffusion-output footprint (it is an attack on the watermark, not invisibility). No public SynthID verifier exists, so success cannot be confirmed locally. It does NOT remove the visible Gemini diamond at strength 0.10; run `wmr remove --synthid --synthid-attack regen` (visible reverse-blend first, then regen) for both.
...
- **Honesty reiterated.** This is still a lossy img2img regen, not verifiable removal. The CoreML and CPU backends produce similar outputs (inter-backend mean diff ~1.23/255). No public SynthID verifier exists. Performance may improve with future GPU/ANE placement work (the UNet likely runs on CPU today).
