...
- **Tradeoffs, stated plainly.** It is lossy (typical output 38 to 45 dB PSNR versus the input). It downloads ~6.5 GB (SDXL base checkpoint) + ~335 MB (fp16-fix VAE) on first use, SHA256-pinned to the HuggingFace LFS content oids and cached in `~/.cache/wmr/`; the model and VAE are NOT bundled. It leaves a detectable diffusion-output footprint (it is an attack on the watermark, not invisibility). No public SynthID verifier exists, so success cannot be confirmed locally. It does NOT remove the visible Gemini diamond at strength 0.05; run `wmr remove --synthid --synthid-attack regen` (visible reverse-blend first, then regen) for both.
- **Tiled img2img preserves up to 4K.** Tiles overlap and cosine-feather so seams are not visible; `--regen-no-tile` forces whole-image (fails above ~1024px on most GPUs). Knobs: `--regen-strength` (0.02 to 0.15, default 0.05), `--regen-steps` (default 20), `--regen-no-download`, `--regen-model-path` / `--regen-vae-path`.
