...
- **SynthID verifier fixtures:** `reference-images/896x1200-gemini36/` = alpha
  calibration fixtures (white/black split + diamond, near-duplicates differing
  only in the carrier) - oracle-hostile, never use for verifier rounds.
  `reference-images/2816x1536-gemini/` = lunar scenes, luma ~85-87
  (restore-gated dim). `test-images/poster-artnight.png` = 1792x2400 Gemini
  generation whose visible mark is NOT detectable (buried in white content) but
  which carries SynthID and verifies cleanly - the bright high-MP fixture.
- **Precedence:** `--rect` > `--geo-preset` > auto-detect > model. New flags on
  `remove`/`detect` (the `visible` subcommand was removed in 1.16.6):
  `--rect x,y,w,h` (shared `parse_rect` helper, also
...
### Detail-restoration (post-regen; band-split default since 1.17.1)

After SynthID regen produces R, `wmr::restore_detail(O, R, cfg)` (`src/core/regen_restore.{hpp,cpp}`, `#ifdef WMR_BUILD_REGEN`) restores regen-lost detail. **Default since 1.17.1: the band-split restore** `out = O - GaussianBlur(O - R, sigma)` (sigma 1.95; == `blur(R) + O - blur(O)`): low band from R, the ENTIRE high band transplanted from O. Recovers ~2x the SSIM of the old sparse restore (6/6 visual passes, zero artifacts), cleared the Google verifier on bright images ~1 MP to 4.3 MP. Formulation from the DeSynth/Synthid-Bypass external investigation (both unlicensed; idea reimplemented, no code copied). `--regen-restore-band-sigma` tunes it: **LARGER sigma restores MORE of the watermarked original (sigma -> inf = O), so lower is safer**; 0 selects the legacy path (top-5%-magnitude diff pixels after a characterized-carrier Wiener, `kissfft`, gamma=4, capped). **Luminance-gated by default** (`RestoreMode::Auto`): bright (mean luma >= 128) restores; dim skips (full regen). The gate is MANDATORY, not conservatism: forced restores on dim images were verifier-DETECTED at sigma 0.95/1.95/2.90 on one image (and 0.95 cleared on another - per-image, unknowable upfront; no in-process verifier). `--regen-restore-detail` forces on (at your own risk); `--no-regen-restore-detail` forces off. It *dilutes* the watermark below the aggregate detection threshold, it does not remove it. The luminance gate is empirical (10-image clearance study; carrier SNR scales ~1/luminance). See `docs/research/synthid-diff-restoration-analysis.md`. The multi-tile path must `.clone()` the pre-regen original before writing R back (a shallow copy silently overwrote O with R, making restoration a no-op on >1024px images; the unit test missed it because it used separate buffers). `external/kissfft/` (BSD-3): float build, the inverse has no 1/N scaling, so `regen_restore.cpp` applies it explicitly to match numpy's `ifft2`. Regen is byte-deterministic (CoreML seed 42; verified stable across e5rt cache recompiles), so repeated-run A/Bs are valid. TRAP: a no-flags `synthid` run RESTORES on bright images (Auto); a pure-regen baseline for an A/B must pass `--no-regen-restore-detail` explicitly. Restore branch logs go to stdout (spdlog), not the stderr progress stream. Evaluation record: `docs/research/bandsplit-restore-evaluation.md`.
...
- **Broken CommandLineTools SDK (link fails with `tapi error: malformed file` / `unknown architecture arm64e.x1-macos`):** xcode-select points at CLT whose SDK tbds the current linker cannot parse. Build via Xcode's SDK instead: `SDKROOT=/Applications/Xcode.app/Contents/Developer/Platforms/MacOSX.platform/Developer/SDKs/MacOSX26.5.sdk EXTRA_CMAKE_ARGS="-DCMAKE_OSX_SYSROOT=$SDKROOT" scripts/build.sh` (DEVELOPER_DIR does NOT work, the shims ignore it; build.sh's EXTRA_CMAKE_ARGS exists for this). Durable fix is the user's: `sudo xcode-select -s /Applications/Xcode.app` or a CLT reinstall.
