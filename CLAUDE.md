...
- **HF model uploads (`froggeric/wmr`):** use `hf upload froggeric/wmr <local> <path-in-repo>` (NOT `huggingface-cli`, which is deprecated and no-ops); it auto-uses the cached `~/.cache/huggingface/token` and auto-tracks `.safetensors`/`.bin` as LFS. Mirroring exact upstream bytes keeps the LFS content-oid (= the SHA256 pin) unchanged. The model-card `license_name` must be lowercase; prefer `license: creativeml-openrail-m`.
- **CoreML model re-pin + the cache fast path (gap until #43 ships):** `ensure_coreml_models` (`coreml_sd_model_fetch.cpp`) short-circuits on file EXISTENCE on the warm path with NO SHA check (the SHA check only runs in `fetch_model_file` on the cold path), so re-pinning a CoreML model SHA does NOT reach already-cached users (adoption + rollback silently miss them). #43 fixes this (always verify, or a sidecar marker); until then a cached user must `rm -rf ~/.cache/wmr/coreml-sdxl` to pick up a re-pinned model.
