...
- **SynthID validation oracle:** Google's "Verify with SynthID" detector is the only ground truth — rate-limited (~10 checks/day/account), natural-language, and noisy (documented hallucinations; went 2/3 in third-party tests). No public verifier exists. Plan "did it clear?" sweeps around the ~10/day budget, and don't over-interpret a single verdict on near-identical images (it may be detector variance).
...
- **sdcpp (regen) CI gaps:** linux configure fails `Could NOT find Vulkan (missing: Vulkan_LIBRARY glslc)` (ggml's Vulkan backend; ubuntu runner has neither); mac x86_64 cross-build fails `unknown target CPU 'apple-m1'` (ggml detects the arm64 host). v1.15.0 set `WMR_BUILD_REGEN=OFF` on linux/windows/tests/mac-x86_64 to ship; mac arm64 builds both `WMR_BUILD_REGEN=ON` + `WMR_BUILD_AI_COREML_SD=ON`. Restoring cross-platform CPU regen (CPU-only sdcpp on linux/windows + a ggml cross-compile patch on x86_64) is plan Task 2 at `~/.claude/plans/post-v1.15-followups.md`.
