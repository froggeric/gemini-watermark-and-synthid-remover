...
- **CoreML model cache fast path (FIXED 1.16.3):** `ensure_coreml_model_files` (`coreml_sd_model_fetch.cpp`) SHA-verifies each cached archive against its current pin via an O(1) sidecar `<archive>.sha256.ok` (the warm path no longer trusts existence alone), so a re-pinned UNet/embeds DOES reach already-cached users. On a re-pin it removes the old archive and extracted `.mlpackage` **before** downloading the new one (peak disk ~= steady state, no accumulation). The embeds are byte-identical across UNet variants, so `kSha256Embeds` is unchanged. Separately, CoreML keeps its own compile caches: the app-scoped e5rt execution cache `~/Library/Caches/wmr/com.apple.e5rt.e5bundlecache` (observed ballooning to **~138 GB** across model versions/tile configs, with no eviction API — far beyond the ~5 GB once estimated) AND the shared `~/Library/Caches/CoreML` (across all CoreML apps). A stale/partial entry there surfaces as `The file "manifest.plist" couldn't be opened` + `Unable to load MPSGraphExecutable ... @ GetMPSGraphExecutable` (non-fatal; CoreML recovers and recompiles, the regen still completes). **Since 1.16.7 the app-scoped `com.apple.e5rt.e5bundlecache` is auto-managed** (cleared on staleness/bloat by `coreml_cache.cpp`; see the "CoreML cache management" section), and `wmr cache --clear-coreml` clears it on demand. Only the shared `~/Library/Caches/CoreML` is still manual: `rm -rf ~/Library/Caches/CoreML` (the next run recompiles, one-time). These are Apple framework `os_log`/`nslog` lines, not suppressible from wmr (unlike FFmpeg's `av_log_set_level`).
...
CLI11 subcommands in src/cli/: `remove` (default), `synthid`, `detect`, `video`, `cache`. (`visible` was removed in 1.16.6; use `remove`.) Directory inputs to remove/synthid trigger batch mode (sequential, outputs to `cleaned/` subdirectory).
...
- Verify a feature's OFF build is clean: `cmake --preset mac-homebrew-Debug -DWMR_<FEATURE>=OFF -B build-off && cmake --build build-off --target wmr`, then `nm build-off/wmr | grep <feature>` (symbols) and `otool -L build-off/wmr | grep <dep>` (e.g. curl). `mac-homebrew-Debug` exists alongside `-Release`; `build-*/` is gitignored.
