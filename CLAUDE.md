...
- **CoreML model cache fast path (FIXED 1.16.3):** `ensure_coreml_model_files` (`coreml_sd_model_fetch.cpp`) SHA-verifies each cached archive against its current pin via an O(1) sidecar `<archive>.sha256.ok` (the warm path no longer trusts existence alone), so a re-pinned UNet/embeds DOES reach already-cached users. On a re-pin it removes the old archive and extracted `.mlpackage` **before** downloading the new one (peak disk ~= steady state, no accumulation). The embeds are byte-identical across UNet variants, so `kSha256Embeds` is unchanged. The caches wmr cannot safely auto-clean are CoreML's own compile caches: `~/Library/Caches/CoreML` (shared across CoreML apps) AND the app-scoped e5rt execution cache `~/Library/Caches/wmr/com.apple.e5rt.e5bundlecache` (observed ballooning to **~138 GB** across model versions/tile configs, with no eviction API — far beyond the ~5 GB once estimated). A stale/partial entry there surfaces as `The file "manifest.plist" couldn't be opened` + `Unable to load MPSGraphExecutable ... @ GetMPSGraphExecutable` (non-fatal; CoreML recovers and recompiles, the regen still completes). Manual clear when noisy or to reclaim disk: `rm -rf ~/Library/Caches/CoreML ~/Library/Caches/wmr/com.apple.e5rt.e5bundlecache` (the next run recompiles, one-time). These are Apple framework `os_log`/`nslog` lines, not suppressible from wmr (unlike FFmpeg's `av_log_set_level`). Follow-up worth considering: wmr could auto-clean the app-scoped `com.apple.e5rt.e5bundlecache` on a model re-pin (it lives under wmr's own cache dir, so it is safe to remove; only `~/Library/Caches/CoreML` is shared).
