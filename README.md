...
| `--regen-restore-band-sigma` | band-split restore sigma (default 1.95; lower = safer, 0 = the previous Wiener + top-5% path) |
...
**What you should expect.** At the strength that reliably scrubs SynthID (0.10) the regen output is visibly smoothed, but on bright images the default detail-restoration (see below) transplants the original's fine detail back, so faces, text, and texture survive far better than a plain regen; dim images (mean luminance below 128) are left fully regenerated, because restoring them has been detected by Google's verifier in testing. Measured fidelity for the raw regen is about 29 to 41 dB PSNR versus the input; the restore recovers most of the fine detail on bright content (about twice the perceptual gain of the restore shipped before 1.17.1). Lowering the strength is **not recommended**: lighter strengths (0.04 to 0.08) cleared singly-watermarked images in testing but missed a double-watermarked image, so 0.10 is the default. Regen also leaves a forensic footprint (a regen output is itself a detectable diffusion output); this is an attack on the watermark, not an invisibility guarantee.
...
Regen is lossy. A **detail-restoration** pass recovers much of that fidelity on the content where it is safe. Since 1.17.1 the default is a **band-split restore**: after regen produces `R`, the output is `O - blur(O - R, sigma)` with sigma 1.95, which keeps the low frequencies from the regeneration (where the watermark lived and was destroyed) and transplants the original's entire high-frequency band back (grain, pores, fabric weave, glyph edges). It recovers roughly twice the perceptual fidelity of the previous Wiener + top-5% restore, was preferred over it in 6/6 blind visual comparisons with zero observed artifacts, and cleared Google's verifier on bright images from ~1 MP up to 4.3 MP. `--regen-restore-band-sigma` tunes it (lower = safer; 0 selects the previous Wiener + top-5% path).

Two findings from [`synthid-diff-restoration-analysis.md`](docs/research/synthid-diff-restoration-analysis.md) and [`bandsplit-restore-evaluation.md`](docs/research/bandsplit-restore-evaluation.md) drive the design:

1. **Restored watermark energy is what the detector keys on.** Restored content re-imports its carrier; staying under the detector's aggregate threshold is a budget, not a guarantee of removal. The band-split spends that budget on the frequencies above the characterized carrier's dominant band (the carrier's magnitude envelope falls as r^-1.3 with its data concentrated at low-mid frequencies).
2. **Mean luminance predicts whether restoration stays clear.** A 10-image clearance study found brightness cleanly separates clear from fail (the 4 dimmest images fail; the 6 brightest clear; threshold about 128, 10/10, monotone), because the carrier's signal-to-content ratio scales as roughly 1/luminance. Forcing restoration past the gate on dim images was verifier-detected at every tested band-split sigma on one test image (and cleared only at the weakest sigma on another), so the gate is mandatory, not a heuristic.

The shipped behavior is **automatic and luminance-gated**: bright images (mean luminance >= 128) get the band-split restoration; dim images (< 128) get full regen (no restoration). Override with `--regen-restore-detail` (force restoration on; you accept the documented risk) or `--no-regen-restore-detail` (force full regen, guaranteed removal).
...
- Safe detail restoration for dim images. The luminance gate routes dark images to full regen because forced restores there have been verifier-detected; the detection is per-image (a weak band-split restored one dim test image cleanly and was detected on another), and with no in-process verifier there is no safe way to pick the images that tolerate it today.
