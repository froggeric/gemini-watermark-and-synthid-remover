...
Gemini image / Veo video carry **SynthID-Image**, a **content-conditional neural watermark** (arXiv 2510.09263): encoder `f(image, payload) -> image`, per-image, detected by a trained extractor via conformal p-value. Not a fixed carrier. The only official open code is `google-deepmind/synthid-text` (text-only). There is **no public SynthID-Image verifier** (Gemini in-app tool: account-gated, ~10 image checks/24h, natural-language output), so removal can never be objectively validated by us. The invisible carrier amplitude is ~0.025/255 (sub-LSB), below the content noise floor. SynthID is now multi-vendor (Google licensed it; OpenAI GPT-image carries it).
