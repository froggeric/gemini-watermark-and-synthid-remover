...
TEST_CASE("consistency_bgr discriminates carrier vs content bins once DC is excluded from max_std",
          "[codebook][synthid]") {
    // Regression test for the consistency_bgr carrier-selection metric.
    //
    // The metric is the discriminative carrier score
    //   consistency = normalize( log1p(mean_magnitude) * phase_coherence )
    // where phase_coherence is the cross-capture mean resultant length
    // |mean_k(exp(i*phase_k))| and the normalize step divides by the max score
    // outside the DC neighborhood. This replaced the old 1 - std/max_std metric
    // which saturated at ~0.999 across ~100% of bins on near-identical captures
    // (per-bin std was tiny everywhere relative to the global max, so the gate
    // was a near-no-op and the builder marked everything as carrier).
    //
    // This test builds a synthetic codebook from N captures where:
    //   - a stable "carrier" frequency is a constant-amplitude cosine (fixed
    //     phase across captures -> high phase_coherence -> high score),
    //   - every other bin carries independent random content per capture
    //     (random phase -> low phase_coherence -> low score).
    // Post-fix the carrier bin's consistency must be high and a typical content
    // bin's consistency must be low, so the subtractor's gate keeps the carrier
    // and drops the content.

    constexpr int W = 64, H = 64;
    constexpr int N = 10;

    // A stable "carrier" frequency, off-axis and well outside the DC mask
    // radius (radius 4 -> bins within 4px of any corner are masked). (10,8)
    // has radius ~12.8.
    constexpr int stable_x = 10, stable_y = 8;
    static_assert(stable_x > 5 && stable_y > 5, "stable bin must be outside DC mask");

    // A few off-axis control bins where only random content lands.
    const int ctrl_bins[][2] = {{22, 18}, {5, 30}, {40, 50}, {15, 45}, {50, 20}};

    const float stable_amp = 40.0f;  // strong enough to clear the content floor
    ...
    for (int i = 0; i < N; ++i) {
        ...
        for (int y = 0; y < H; ++y) {
            for (int x = 0; x < W; ++x) {
                // Independent random content per pixel per capture: its FFT
                // phase is incoherent across captures at every bin.
                int v = uval(rng);
                // Fixed additive carrier (identical across captures): its FFT
                // phase is stable at (stable_x, stable_y).
                const float ph_stable =
                    2.0f * static_cast<float>(CV_PI) *
                    (stable_x * x + stable_y * y) / static_cast<float>(W);
                v = std::clamp(static_cast<int>(v + stable_amp * std::cos(ph_stable)), 0, 255);
    ...
    // Read consistency at the carrier bin and the control bins (channel 0; BGR
    // are identical on grey input).
    const float cons_stable = prof.consistency_bgr[0].at<float>(stable_y, stable_x);
    double cons_ctrl_sum = 0;
    for (const auto& b : ctrl_bins) {
        cons_ctrl_sum += prof.consistency_bgr[0].at<float>(b[1], b[0]);
    }
    const float cons_ctrl = static_cast<float>(cons_ctrl_sum / 5);

    INFO("carrier bin (" << stable_x << "," << stable_y << ") consistency=" << cons_stable
         << " | control bins mean consistency=" << cons_ctrl
         << " | samples=" << prof.sample_count);

    // Carrier bin kept (phase-coherent -> high score), content bins dropped
    // (phase-incoherent -> low score). This is the discrimination the gate
    // provides; reverting to the saturated 1 - std/max_std metric makes every
    // bin read ~0.99 and the cons_ctrl check fails.
    REQUIRE(cons_stable > 0.6f);
    REQUIRE(cons_ctrl < 0.4f);
